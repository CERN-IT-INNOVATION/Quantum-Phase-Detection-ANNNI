    ...
    def train(self, lr, n_epochs, reg=0, circuit=False, recycle=True, epochs_batch_size = 500, batch_size = 100, excited = False, beta = 0):
        ...
        if excited:
            ...
            try:
                ...
                params = np.array( np.random.uniform(-np.pi, np.pi, size=(self.n_states,self.n_params)) )
        ...
        if not recycle:
            ...
            for batch in batches:
                ...
                if excited:
                    outers = jv_psi_outer(psi0_batch)
            ...
            H_effective = np.real(qml.matrix(self.Hs.qml_Hs[idx])).astype(np.single)
            ...
            E_state = j_compute_vqe_E(previous_state, H_effective)
            vqe_E[idx] = E_state
            params[idx] = copy.copy(param)
            progress.set_description("{0}/{1} | Cost: {2}".format(idx + 1, self.n_states, np.abs(E_state - self.Hs.true_e0[idx])/np.abs(self.Hs.true_e0[idx]) ))
            ...
            for idx in (progress):
                ...
                if excited:
                    gs_psi = j_q_vqe_state(self.vqe_params[idx])
                    H_effective += beta * j_psi_outer(gs_psi)
...
def load_vqe(filename):
    ...
    if len(things_to_load) == 4:
        Hs, vqe_params, vqe_e, circuit_fun = things_to_load
    else:
        Hs, vqe_params, vqe_e, vqe_params1, vqe_e1, circuit_fun = things_to_load
        loaded_vqe.vqe_params1 = vqe_params1
        loaded_vqe.vqe_e1 = vqe_e1
    loaded_vqe = vqe(Hs, circuit_fun)
    loaded_vqe.vqe_params = vqe_params
    loaded_vqe.vqe_e = vqe_e
