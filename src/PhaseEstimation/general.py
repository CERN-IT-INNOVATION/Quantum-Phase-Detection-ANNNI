...
def geteigvals(qml_H : qml.ops.qubit.hamiltonian.Hamiltonian, states : List[int]) -> List[Number]:
    """
...
def get_H_eigval_eigvec(qml_H : qml.ops.qubit.hamiltonian.Hamiltonian, en_lvl : int) -> Tuple[List[List[Number]], Number, List[Number]]:
    """
...
def get_VQE_params(qml_H : qml.ops.qubit.hamiltonian.Hamiltonian) -> Tuple[List[List[Number]], Number]:
    """
...
def get_VQD_params(qml_H : qml.ops.qubit.hamiltonian.Hamiltonian, beta : Number) -> Tuple[List[List[Number]], List[List[Number]], Number]:
    """
...
def get_neighbours(vqeclass : vqe.vqe, idx : int) -> List[Number]:
    """
    ...
    if (idx + 1) % side == 0 and idx != vqeclass.Hs.n_states - 1:
